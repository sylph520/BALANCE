...
lr=(0.00005 0.00025 0.005)
nsteps=(64 128 256 512)
entcoef=(0 0.1 0.2)
    ...
    for  ec in "${entcoef[@]}"; do
      for cr in "${cliprange[@]}"; do
        for ga in "${gamma[@]}"; do
          ...
          python main.py --config $conf_file --fix_index_count $idxc --ts 100000 --tb_log tensor_log/gridsearch --lr $lr --ns $ns --ec $ec --cr $cr --gamma $ga > logs/$log_fn
        done
      done
    done
  done
