...
usage() {
  cat <<'EOF'
Usage: run_batch.sh --bm <benchmark> [--ts timesteps] [--imax N] [--model_path /abs/path/to/model.zip] [--mode_id run_folder]
  ...
  --mode_id     TensorBoard run folder; expands to tensor_log/<mode_id>/final_model.zip.
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --mode_id)    [[ $# -gt 1 ]] || usage; MODE_ID="$2"; shift 2;;
    -h|--help)    usage;;
...
if [[ -n "$MODEL_PATH" ]]; then
  ...
  MODEL_ARG=(--load_model "tensor_log/${MODE_ID}/final_model.zip")
fi
...
for workload_path in "${workloads[@]}"; do
  ...
  python main.py \
    --config experiments/tpcdsc_conf/tpcdsc_index_count_tune.json \
    --test_only \
    --fix_index_count 5 \
    --ts "$TS" \
