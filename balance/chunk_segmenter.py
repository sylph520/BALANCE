...
def segment_workloads(workload_stream, difference_threshold_x, hash2tid: dict, dbname: str):
    ...
    for i in workload_stream:
        w_tids = workload2tids(i, hash2tid, dbname, unify_similar_ops=True)
        w_tids = set(w_tids)
...
def workload2tids(workload: Workload, hash2tid: dict, dbname: str, unify_similar_ops: bool) -> List[int]:
    workload_tids: List[int] = []
    for q in workload.queries:
        h, _ = query_to_hash(q.text, dbname=dbname, unify_similar_ops=unify_similar_ops)
        tid =  hash2tid[h]
        workload_tids.append(tid)
    return workload_tids


def workload_fits_chunk(chunk_workloads: List[Workload], worklaod_new: Workload, difference_threshold_x,
                        hash2tid, dbname, unify_similar_ops) -> bool:
    chunk_tids = set(workload2tids(chunk_workloads[0], hash2tid, dbname, unify_similar_ops))
    nw_tids = set(workload2tids(worklaod_new, hash2tid, dbname, unify_similar_ops))
    new_tids = nw_tids - chunk_tids
    diff_percent = len(new_tids) / len(chunk_tids) * 100
    if diff_percent > difference_threshold_x:
        return False
    else:
        return True
